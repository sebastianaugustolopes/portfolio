  ...
  const handleImageChange = useCallback((e) => {
    ...
    if (file) {
      ...
      if (file.size > 5 * 1024 * 1024) {
        alert(
          "O tamanho do arquivo deve ser menor que 5MB. Por favor, escolha uma imagem menor."
        );
        // Reset the input
  ...
  const handleCommentSubmit = useCallback(
    async ({ newComment, userName, imageFile }) => {
      ...
      try {
        ...
        console.log("Image processed:", profileImageUrl);
        ...
        // Update the comments list manually
        ...
        setError("Falha ao publicar comentário. Por favor, tente novamente.");
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-xl font-semibold text-white">
            Comentários{" "}
            <span className="text-indigo-400">({totalComments})</span>
          </h3>
          <button
            onClick={() => {
              const fetchComments = async () => {
            ...
            className="ml-auto px-3 py-1 text-xs bg-indigo-500/20 text-indigo-400 rounded-lg hover:bg-indigo-500/30 transition-all"
          >
            Atualizar
          </button>
        </div>
